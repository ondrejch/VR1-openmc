...
thermal_default_params: dict = {'beta': np.array([0.000215, 0.00142, 0.00127, 0.00257, 0.00075, 0.00027]),
    'lambda_': np.array([0.0126, 0.0337, 0.139, 0.325, 1.13, 2.50]), 'Lambda': 5e-4}

fast_reactor_params: dict = {'beta': np.array([0.00022, 0.00142, 0.00127, 0.00257, 0.00075, 0.00027]),
    'lambda_': np.array([0.0126, 0.0337, 0.139, 0.325, 1.13, 2.50]), 'Lambda': 1e-7}


class PointKineticsEquationSolver:
    def __init__(self, reactivity_func, params=None):
    ...
    def solve(self, t_span=(0, 10), t_eval=None):
        ...
        def equations(t, y):
            n, *C = y
            rho = self.reactivity_func(t)
            prompt = (rho - beta.sum()) / Lambda
            return [n * prompt + np.dot(lambda_, C),
                *(beta[i] / Lambda * n - lambda_[i] * C[i] for i in range(len(C)))]
